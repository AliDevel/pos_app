...
export default {
  ...
  methods: {
    ...
    get_search(first_search) {
      ...
      if (
        first_search &&
        first_search.startsWith(this.pos_profile.posa_scale_barcode_start) || first_search.startsWith(this.pos_profile.posa_scale_barcode_start )  || first_search.startsWith(this.pos_profile.scale_barcode_start_with_2 )  )    {
    ...
    scan_barcoud() {
      ...
      trigger_onscan: function trigger_onscan(sCode) {
       var name = '';
        ...
        var isCustomer=JSON.parse(localStorage.customer_storage).filter(function (entry) {
      ...
      if(name === ''){
       ...
       if (this.filtred_items.length == 0) {
          ...
          // TODO: Rewrite in more correct way

           Object.keys(isCustomer).length >0 ? evntBus.$emit('set_customer', name) : '';
           this.first_search = ''; 
          //this.debounce_search = null;
         // this.search = null;
        }    },
